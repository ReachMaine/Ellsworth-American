<?php
/* modifications
	11Sept15 zig - add is_mobile & is_desktop shortcodes
 */
/* custom functions for Ellsworth American, Inc */
/* Embed  related posts into content */
/* changed minds:  accross bottom, if less that 6 paragraph tags */
/* check for disabled */
function embed_related_content($in_placement="") {

	$zcontent = apply_filters('the_content', get_the_content());
	if ($in_placement == 'Disable'){
		echo $zcontent;
	} else {
		if ($zcontent) { // check for empty content
			...
			/* echo '<!--- ptagcount = '.$ptagcount.'--->'; */
			if ( ($ptagcount < 6) || ($in_placement=="Bottom") || ($in_placement == "bottom") ) { /* run accross bottom, if less than 6 p-tags */
				echo $zcontent;
				...
				if ($thirdp) { // check for no third p tag
					...
					echo '</div></div>';
					echo $zcontent2;
				} else {  /* shouldnt ever get here but JIC */
...
<?php
	...
	function eai_do_feat($arg_meta = false, $arg_excerpt=false) {
		$num_excerpt = 15;
		?>
		<div class="prl-span-9 ea_feat <?php echo get_the_ID(); ?>">
           <div class="prl-grid">
	...
	add_shortcode('is_mobile', 'zig_is_mobile');
	...
	add_shortcode('is_desktop', 'is_desktop_funct');
	function is_desktop_funct($atts, $content) {
		if (!wp_is_mobile()) {
			return $content;
		} else {
			return "";
