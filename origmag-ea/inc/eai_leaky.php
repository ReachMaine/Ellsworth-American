...
/*
	mods:
		...
		28july15 zig - change eai_leaky_loginmenu() s.t. only have one link in header (cache-ing problem).
		31july15 zig - add logoutbutton shortcode & make links on membership pages look like buttons (add class="prl-button")
*/
...
if ( !function_exists( 'do_eai_paywall_welcome') ) {
	function do_eai_paywall_welcome() {
		global $current_user;
		$eai_this_paper = "The Ellsworth American";
		$eai_circ_phone = "207 667-2576";
		$eai_circ_email = "membership@ellsworthamerican.com";
		$eai_with_subscript_get = '<h3 class="eai-with-head">With your subscription, you get...</h3>[template id="107841"]';
		$result = "";
		...
		if ($eai_user != 0) {
			...
			$result .= '<p>Click here to <a class="prl-button" href="'.wp_logout_url( $logout_url ).'">Log Out</a>';
			...
			if (function_exists( 'is_issuem_leaky_subscriber_logged_in') ) {
				 if (is_issuem_leaky_subscriber_logged_in()) {
					...
					if ( !empty($exp )) {
				 		...
				 		$result .= '<p class="eai-ng-explain"> According to our records, your subscription to <i>'.$eai_this_paper.'</i> has expired or is not longer valid. ';
...
/*
	function to display the my account in header (not login/subscribe or hello/logout  anymore).
*/
