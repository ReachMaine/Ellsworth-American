...
function post_meta($date = true,$author=true,$comment=true,$cat=true,$view = true, $updated=false){?>
	<div class="prl-article-meta">
		<?php 
		if($date == true) { echo '<span>'; the_time('F j, Y');  echo '</span>'; } ?>
		<?php if (( $author==true) && (!get_post_meta( get_the_ID(), 'ts_fab_hide', false ))  ) {?>
			<span><?php _e('by', 'presslayer');?> <a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author"><?php echo get_the_author(); ?></a></span>
		<?php } ?>
...
function post_thumb($post_id, $width, $height = FALSE, $crop = FALSE){ 
	if ( has_post_thumbnail()):
		...
		if ( $format != 'file') {
			$str .= '<span class="prl-overlay-area o-'.$overlay[$format].'"></span>';
		}
		$str .= '</span></a>';
		return $str;
	endif;	
