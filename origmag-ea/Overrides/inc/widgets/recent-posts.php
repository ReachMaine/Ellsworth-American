...
class recents_posts_Widget extends WP_Widget {
	...
	function widget($args, $instance)
	{
		...
		<?php
		if($category > 0){
			echo $before_title.'<a href="'.get_category_link( $category ).'">';
			if ($title) {
				echo $title;
			} else {
				echo get_the_category_by_ID( $category);
			}
			echo '</a><span class="prl-block-title-link right"><a href="'.get_category_link( $category ).'">'.__('All posts','PressLayer').' <i class="fa fa-caret-right"></i></a></span>'.$after_title;
		}else{
