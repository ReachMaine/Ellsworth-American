<?php
/* mod notes: 
	8Sept2014 zig - add to all link after uber menu.
	26Dec14 zig - add post_type = 'any' to the mega menu wp_query.
*/
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if($magamenu == true) {
			...
			if ($wpdb->num_rows > 0) {
				...
				$terms = get_terms( 'category', array('orderby' => 'count','order' => 'DESC','hide_empty' => 0, 'parent' => $item->object_id) );
			...
			$megapost = new WP_Query(array('showposts' => $showposts,'cat' => $item->object_id, 'post_type' => 'any'));
			...
			while($megapost->have_posts()): $megapost->the_post();
				...
				if( has_post_thumbnail()):
					...
					if ($format != 'file') {$item_output  .= '<span class="prl-overlay-area o-'.$overlay[$format].'"></span>';}
					$item_output .= '</span></a>';
				endif;
			...
			$item_output .= '<div class="eai-mega-all"><a '.$attributes.'>See all'.'</a></div>'; 
			$item_output  .= '</div>';
