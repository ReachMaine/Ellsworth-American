...
/*
	zig changes
		- add redirect parameter to login  [leaky_paywall_login]
		- add username to login form lable.
		- comment out lost email password 
*/
...
if ( !function_exists( 'do_leaky_paywall_login' ) ) { 
	 ...
	 */
	/*
		22jan15 - zig - add parameter for readirect
	*/
	function do_leaky_paywall_login( $atts ) {
		...
		if ( 'passwordless' === $settings['login_method'] ) {
			...
			/* add_action( 'login_form_bottom', 'leaky_paywall_add_lost_password_link' ); zig - commented out, use our own */ 
			$args = array(
