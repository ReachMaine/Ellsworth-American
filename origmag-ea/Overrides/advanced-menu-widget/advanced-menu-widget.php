...
*/
/* mods 
 * 	17May16   zig -  add parameter to shortcode for 'Select' text (shortcode only, for now)
 *		  		  - replace parent::WP_Widget  with parent::__construct
 */
 ...
 */
 class Advanced_Menu_Widget extends WP_Widget {
	...
	function widget($args, $instance) {
		$choose_text = "Select";
		if ( $instance['stext'] ) {
			$choose_text = $instance['stext'];
		} 
		$items_wrap = !empty( $instance['dropdown'] ) ? '<select id="amw-'.$this->number.'" class="%2$s amw" onchange="onNavChange(this)"><option value="">'.$choose_text.'</option>%3$s</select>' : '<ul id="%1$s" class="%2$s">%3$s</ul>';
